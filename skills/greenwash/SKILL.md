---
name: greenwash
description: Use this skill before telling the user a task is complete, a bug is fixed, tests pass, or anything else that amounts to "done" after writing or changing code. Also use it any time you're about to summarize a test run instead of showing it. Trigger this even if you're confident the fix is correct -- confidence is not the thing being checked. It requires pasting the literal command and literal output as proof, and running a check that scans the diff for the specific ways agents fake a pass (skipped tests, mocking the unit under test, hardcoded return values, swallowed exceptions, sys.exit(0), conftest.py patching, AlwaysEqual overrides) -- and that can run the tests themselves -- before the "done" claim goes out.
---
...
```bash
python "${CLAUDE_PLUGIN_ROOT}/scripts/greenwash_check.py" scan
```
...
```bash
python "${CLAUDE_PLUGIN_ROOT}/scripts/greenwash_check.py" verify \
...
If this plugin's Stop hook is enabled, the check also runs automatically every
time you try to end your turn, and reports what it found. By default it reports
without blocking: you will see greenwash in the transcript, and the rules above
still apply -- so treat them as how to work, not as a hurdle to clear.

With enforcement switched on (`"mode": "enforce"` in `.greenwash/config.json`,
or `GREENWASH_ENFORCE=1`), a finding blocks the turn instead and the report is
handed back to you. Either way the hook does not know intent, only patterns and
test results; explaining a false positive to the user is still on you.
